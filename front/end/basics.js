...
// functions
/* arrow functions does not inherit `this`; wherever you define them, arrow functions will always see `this` as the previous global object */
...
/// modern class
class Dot {
    x = 0;                  /* initialize the property to a default value */
    y;
    ...
    GoX(dx) { this.x += dx; }
    GoY(dy) { this.y += dy; }
    #isDotOn(x, y) { return x == this.x && y == this.y; }
...
Line.prototype = Object.create(Dot.prototype);  /* line prototype inherits from dot */
...
class AnArray extends Array {
    constructor(...args) {
        ...
        /* If a subclass has any of its own initialization to do, it must first call the superclass constructor
...
let thymap = new Map([["K1", "V1"], ["K2", "V2"]]);
...
let abc = Int32Array.from([0, 1, 2, 3]);           /* create a typed array from an array or iterable object */
/* set() method */
